...
def main() -> None:
    ...
    for name, text in [('guard', guard), ('router', router)]:
        ...
        if 'QUALITY B HARD-LOCK V7' in text:
            raise SystemExit(f'{name.upper()}_OLD_HARD_LOCK_REMAINS')
    if 'qualitySafeB' in router:
