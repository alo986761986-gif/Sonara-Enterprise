...
async function resolvePrincipalEmail(token: string) {
  ...
  if (credentialsPath) {
    try {
      ...
      const email = String(parsed?.client_email || parsed?.service_account_impersonation_url || '').trim();
      if (email.includes('@')) return { email, source: 'GOOGLE_APPLICATION_CREDENTIALS' };
    } catch {}
  }
...
export async function repairTranscoderIam(app: App) {
  ...
  if (!principal) {
    return {
      ...
      stage: 'principal',
      project,
      principalSource: principalInfo.source,
      hasGoogleApplicationCredentials: Boolean(String(process.env.GOOGLE_APPLICATION_CREDENTIALS || '').trim()),
      error: 'Runtime Google principal email could not be resolved.'
    };
  }
  ...
  const verified = Boolean(
    verify.response.ok &&
    (verify.data?.bindings || []).some((b: any) => b?.role === role && Array.isArray(b.members) && b.members.includes(member))
  );
