...
export default function DJAudioRouting() {
  ...
  const contextSinkSupported = typeof (AudioContext.prototype as any).setSinkId === 'function';
  ...
  const scan = async () => {
    if (!navigator.mediaDevices?.enumerateDevices) { setStatus('Il browser non espone MediaDevices. Usa SONARA DJ Bridge per il routing audio professionale.'); return; }
    setBusy(true);
  ...
  return <section className="rounded-3xl border border-cyan-500/15 bg-slate-900/50 p-4 sm:p-5">
    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><div className="flex items-center gap-2"><AudioLines className="h-4 w-4 text-cyan-300"/><h2 className="text-sm font-black text-white">Professional Audio Routing</h2></div><p className="mt-1 text-[10px] text-slate-500">Master, booth/cue e input disponibili nel browser. ASIO/CoreAudio/WASAPI multicanale completo passa dal DJ Bridge.</p></div><button onClick={() => void scan()} disabled={busy} className="flex items-center justify-center gap-2 rounded-xl border border-cyan-500/25 bg-cyan-500/10 px-4 py-2.5 text-[9px] font-black text-cyan-100 disabled:opacity-50">{busy ? <Loader2 className="h-3.5 w-3.5 animate-spin"/> : <RefreshCw className="h-3.5 w-3.5"/>} SCANSIONA AUDIO</button></div>
    <div className="mt-4 grid gap-3 lg:grid-cols-3"><label className="rounded-xl border border-slate-800 bg-slate-950/70 p-3"><span className="flex items-center gap-2 text-[9px] font-black text-slate-400"><Volume2 className="h-3.5 w-3.5 text-fuchsia-300"/> MASTER OUTPUT</span><select value={routing.masterOutput} onChange={e => save({ masterOutput: e.target.value })} className="mt-2 w-full rounded-lg border border-slate-800 bg-slate-900 px-2 py-2 text-[9px] text-white"><option value="default">Sistema predefinito</option>{outputs.map((device,index)=><option key={device.deviceId} value={device.deviceId}>{labelFor(device,index)}</option>)}</select><button disabled={!sinkSupported} onClick={() => void testOutput(routing.masterOutput)} className="mt-2 w-full rounded-lg border border-slate-800 bg-slate-900 px-2 py-2 text-[8px] font-black text-slate-400 disabled:opacity-40">TEST MASTER</button></label>
      ...
      <label className="rounded-xl border border-slate-800 bg-slate-950/70 p-3"><span className="flex items-center gap-2 text-[9px] font-black text-slate-400"><Mic2 className="h-3.5 w-3.5 text-amber-300"/> AUDIO INPUT</span><select value={routing.input} onChange={e => save({ input: e.target.value })} className="mt-2 w-full rounded-lg border border-slate-800 bg-slate-900 px-2 py-2 text-[9px] text-white"><option value="default">Sistema predefinito</option>{inputs.map((device,index)=><option key={device.deviceId} value={device.deviceId}>{labelFor(device,index)}</option>)}</select><div className="mt-2 rounded-lg border border-slate-800 bg-slate-900 px-2 py-2 text-center text-[8px] font-black text-slate-500">MIC / LINE CAPTURE</div></label></div>
    <div className="mt-4 grid gap-2 sm:grid-cols-3"><div className="rounded-xl border border-slate-800 bg-slate-950 p-3"><div className="text-[8px] font-black text-slate-600">OUTPUT SELECTION</div><div className={`mt-1 flex items-center gap-1 text-[9px] font-black ${sinkSupported ? 'text-emerald-300' : 'text-slate-600'}`}>{sinkSupported ? <CheckCircle2 className="h-3 w-3"/> : null}{sinkSupported ? 'SUPPORTED' : 'BRIDGE REQUIRED'}</div></div><div className="rounded-xl border border-slate-800 bg-slate-950 p-3"><div className="text-[8px] font-black text-slate-600">AUDIOCONTEXT SINK</div><div className={`mt-1 text-[9px] font-black ${contextSinkSupported ? 'text-emerald-300' : 'text-slate-600'}`}>{contextSinkSupported ? 'SUPPORTED' : 'DEFAULT OUTPUT'}</div></div><div className="rounded-xl border border-slate-800 bg-slate-950 p-3"><div className="text-[8px] font-black text-slate-600">DECLARED LATENCY</div><div className="mt-1 text-[9px] font-black text-white">{latency}</div></div></div>
