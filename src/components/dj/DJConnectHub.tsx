...
type MidiLearnAction =
  | 'deckA.play' | 'deckA.cue' | 'deckA.sync' | 'deckA.pitch' | 'deckA.volume' | 'deckA.filter' | 'deckA.eqLow' | 'deckA.eqMid' | 'deckA.eqHigh'
  | 'deckB.play' | 'deckB.cue' | 'deckB.sync' | 'deckB.pitch' | 'deckB.volume' | 'deckB.filter' | 'deckB.eqLow' | 'deckB.eqMid' | 'deckB.eqHigh'
...
export default function DJConnectHub() {
  ...
  const [status, setStatus] = useState('Pronto per collegare una console DJ.');
  ...
  return <div className="space-y-5 pb-10">
    <section className="overflow-hidden rounded-3xl border border-purple-500/20 bg-[radial-gradient(circle_at_top_right,rgba(109,40,217,.22),transparent_36%),linear-gradient(145deg,#070912,#05070c)] p-5 shadow-2xl sm:p-7">
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-center"><div className="max-w-3xl"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full border border-fuchsia-500/20 bg-fuchsia-500/10 px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.2em] text-fuchsia-200">SONARA DJ PRO</span><span className="rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-1 text-[9px] font-black text-emerald-300">LIVE HARDWARE ENGINE</span><span className="rounded-full border border-cyan-500/20 bg-cyan-500/10 px-2.5 py-1 text-[9px] font-black text-cyan-200">{activeProfile.model}</span></div><h1 className="mt-4 text-2xl font-black tracking-tight text-white sm:text-3xl">Una sola sezione per controller, player, mixer e console DJ.</h1><p className="mt-3 max-w-2xl text-xs leading-6 text-slate-400">SONARA rileva l’hardware reale, carica il profilo più vicino, riceve MIDI/HID, pilota il Live Deck Engine e usa il DJ Bridge per MIDI nativo, audio system APIs e sincronizzazione Ableton Link.</p></div>
        <div className="grid min-w-[300px] grid-cols-2 gap-2"><button onClick={() => void connectMidi()} disabled={busy} className="flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-fuchsia-600 to-purple-600 px-4 py-3 text-[10px] font-black text-white disabled:opacity-50">{busy ? <Loader2 className="h-4 w-4 animate-spin"/> : <Music2 className="h-4 w-4"/>} MIDI</button><button onClick={() => void connectHid()} disabled={busy} className="flex items-center justify-center gap-2 rounded-xl border border-cyan-500/30 bg-cyan-500/10 px-4 py-3 text-[10px] font-black text-cyan-100 disabled:opacity-50"><Usb className="h-4 w-4"/> HID</button><button onClick={bridge.connect} className="flex items-center justify-center gap-2 rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-[10px] font-black text-slate-200"><Cpu className="h-4 w-4"/> BRIDGE · {bridge.state.toUpperCase()}</button><button onClick={() => setLink(!linkEnabled)} className={`flex items-center justify-center gap-2 rounded-xl border px-4 py-3 text-[10px] font-black ${linkEnabled ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-200' : 'border-slate-700 bg-slate-950 text-slate-300'}`}><Link2 className="h-4 w-4"/> LINK {linkEnabled ? 'ON' : 'OFF'}</button></div></div>
