...
import { Disc3, FolderOpen, Gauge, Pause, Play, RotateCcw, SlidersHorizontal, Zap } from 'lucide-react';
...
const makeDeck = (): DeckModel => ({
  title: 'Nessuna traccia', url: '', duration: 0, bpm: 124, playing: false, volume: 0.9, filter: 0, low: 0, mid: 0, high: 0,
  hotCues: [null, null, null, null, null, null, null, null], loopActive: false, loopStart: 0, loopBeats: 4, rate: 1
});
...
function DeckPanel({ id, model, currentTime, onFile, onToggle, onCue, onHotCue, onLoop, onModel }: {
  ...
  return <div className="rounded-2xl border border-slate-800 bg-[#070a10] p-4 shadow-xl">
    <div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="text-[9px] font-black uppercase tracking-[0.25em] text-cyan-300">DECK {id}</div><div className="mt-1 truncate text-sm font-black text-white">{model.title}</div><div className="mt-1 text-[9px] text-slate-500">{seconds(currentTime)} / {seconds(model.duration)} · {model.bpm.toFixed(1)} BPM · {model.rate.toFixed(3)}x</div></div><Disc3 className={`h-9 w-9 shrink-0 text-slate-600 ${model.playing ? 'animate-spin' : ''}`} /></div>
    <div className="mt-4 h-16 overflow-hidden rounded-xl border border-slate-800 bg-slate-950 p-2"><div className="relative h-full overflow-hidden rounded-lg bg-[repeating-linear-gradient(90deg,rgba(148,163,184,.08)_0px,rgba(148,163,184,.08)_2px,transparent_2px,transparent_7px)]"><div className="absolute inset-y-0 left-0 bg-gradient-to-r from-fuchsia-500/25 to-cyan-400/25" style={{ width: `${progress * 100}%` }} /><div className="absolute inset-y-0 w-px bg-white" style={{ left: `${progress * 100}%` }} /></div></div>
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-[8px] font-black text-slate-500">BPM <span className="float-right text-white">{model.bpm.toFixed(1)}</span><input type="range" min={60} max={200} step={0.1} value={model.bpm} onChange={e => onModel({ bpm: Number(e.target.value) })} className="mt-1 w-full accent-cyan-500" /></label><label className="text-[8px] font-black text-slate-500">VOLUME <span className="float-right text-white">{Math.round(model.volume * 100)}%</span><input type="range" min={0} max={1} step={0.01} value={model.volume} onChange={e => onModel({ volume: Number(e.target.value) })} className="mt-1 w-full accent-fuchsia-500" /></label></div>
    <div className="mt-3 grid grid-cols-4 gap-2"><label className="text-[8px] font-bold text-slate-600">LOW<input type="range" min={-18} max={9} step={0.5} value={model.low} onChange={e => onModel({ low: Number(e.target.value) })} className="mt-1 w-full accent-purple-500" /></label><label className="text-[8px] font-bold text-slate-600">MID<input type="range" min={-18} max={9} step={0.5} value={model.mid} onChange={e => onModel({ mid: Number(e.target.value) })} className="mt-1 w-full accent-purple-500" /></label><label className="text-[8px] font-bold text-slate-600">HIGH<input type="range" min={-18} max={9} step={0.5} value={model.high} onChange={e => onModel({ high: Number(e.target.value) })} className="mt-1 w-full accent-purple-500" /></label><label className="text-[8px] font-bold text-slate-600">FILTER<input type="range" min={-1} max={1} step={0.01} value={model.filter} onChange={e => onModel({ filter: Number(e.target.value) })} className="mt-1 w-full accent-cyan-500" /></label></div>
  </div>;
}
...
export default function DJLiveMixer() {
  ...
  const ensureGraph = async () => {
    ...
    const create = (audio: HTMLAudioElement, graphRef: React.MutableRefObject<DeckAudioGraph | null>) => {
      ...
      source.connect(low).connect(mid).connect(high).connect(filterNode).connect(deckGain).connect(crossGain).connect(masterGain.current!);
      graphRef.current = { source, low, mid, high, filter: filterNode, deckGain, crossGain };
  ...
  useEffect(() => { updateGraph(deckA, graphA.current); if (audioA.current) audioA.current.playbackRate = deckA.rate; }, [deckA.volume, deckA.low, deckA.mid, deckA.high, deckA.filter, deckA.rate]);
  useEffect(() => { updateGraph(deckB, graphB.current); if (audioB.current) audioB.current.playbackRate = deckB.rate; }, [deckB.volume, deckB.low, deckB.mid, deckB.high, deckB.filter, deckB.rate]);
  useEffect(() => { if (masterGain.current) masterGain.current.gain.value = master; }, [master]);
  useEffect(() => { const x = clamp((crossfader + 1) / 2, 0, 1); if (graphA.current) graphA.current.crossGain.gain.value = Math.cos(x * Math.PI / 2); if (graphB.current) graphB.current.crossGain.gain.value = Math.sin(x * Math.PI / 2); emitDJFeedback({ control: 'crossfader', value: crossfader }); }, [crossfader]);
  ...
  useEffect(() => onDJControl(action => {
    const apply = (deck: DJDeckId, patch: Partial<DeckModel>) => deck === 'A' ? setDeckA(c => ({ ...c, ...patch })) : setDeckB(c => ({ ...c, ...patch }));
    const audio = action.type.startsWith('deck.') && 'deck' in action ? (action.deck === 'A' ? audioA.current : audioB.current) : null;
    if (action.type === 'deck.play' && audio) { void ensureGraph().then(() => { if (audio.paused) void audio.play(); else audio.pause(); apply(action.deck, { playing: !audio.paused }); }); }
    ...
    if (action.type === 'deck.hotcue' && audio && action.pressed !== false) {
      const model = action.deck === 'A' ? deckA : deckB; const cue = model.hotCues[action.index]; if (cue == null) { const next = [...model.hotCues]; next[action.index] = audio.currentTime; apply(action.deck, { hotCues: next }); } else audio.currentTime = cue;
    }
  ...
  const loadFile = async (deck: DJDeckId, file: File) => {
    ...
    const duration = await new Promise<number>(resolve => { const done = () => resolve(Number.isFinite(audio.duration) ? audio.duration : 0); audio.addEventListener('loadedmetadata', done, { once: true }); window.setTimeout(() => resolve(Number.isFinite(audio.duration) ? audio.duration : 0), 2500); });
    const patch = { title: file.name.replace(/\.[^.]+$/, ''), url, duration, playing: false, hotCues: [null, null, null, null, null, null, null, null] as Array<number | null>, loopActive: false, loopStart: 0, rate: 1 };
    if (deck === 'A') setDeckA(c => ({ ...c, ...patch })); else setDeckB(c => ({ ...c, ...patch }));
  };
  const toggle = async (deck: DJDeckId) => { const audio = deck === 'A' ? audioA.current : audioB.current; if (!audio?.src) return; await ensureGraph(); if (audio.paused) await audio.play(); else audio.pause(); (deck === 'A' ? setDeckA : setDeckB)(c => ({ ...c, playing: !audio.paused })); emitDJFeedback({ control: 'play', deck, value: !audio.paused }); };
  const cue = (deck: DJDeckId) => { const audio = deck === 'A' ? audioA.current : audioB.current; if (!audio) return; audio.pause(); audio.currentTime = 0; (deck === 'A' ? setDeckA : setDeckB)(c => ({ ...c, playing: false })); };
  const hotCue = (deck: DJDeckId, index: number) => { const audio = deck === 'A' ? audioA.current : audioB.current; const model = deck === 'A' ? deckA : deckB; if (!audio) return; const cuePoint = model.hotCues[index]; if (cuePoint == null) { const next = [...model.hotCues]; next[index] = audio.currentTime; (deck === 'A' ? setDeckA : setDeckB)(c => ({ ...c, hotCues: next })); } else audio.currentTime = cuePoint; };
  const loop = (deck: DJDeckId) => { const audio = deck === 'A' ? audioA.current : audioB.current; if (!audio) return; (deck === 'A' ? setDeckA : setDeckB)(c => ({ ...c, loopActive: !c.loopActive, loopStart: audio.currentTime })); };
  const sync = (deck: DJDeckId) => { const own = deck === 'A' ? deckA : deckB; const other = deck === 'A' ? deckB : deckA; if (!own.url || !other.url || !own.bpm || !other.bpm) return; const rate = clamp(other.bpm / own.bpm, 0.5, 2); (deck === 'A' ? setDeckA : setDeckB)(c => ({ ...c, rate })); };

  const latency = useMemo(() => { const ctx = contextRef.current as any; if (!ctx) return '—'; const total = Number(ctx.baseLatency || 0) + Number(ctx.outputLatency || 0); return total > 0 ? `${(total * 1000).toFixed(1)} ms` : 'browser'; }, [audioReady]);

  return <section className="rounded-3xl border border-fuchsia-500/15 bg-[linear-gradient(145deg,#070910,#05070b)] p-4 sm:p-5">
    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><div className="flex items-center gap-2"><Zap className="h-4 w-4 text-fuchsia-300"/><h2 className="text-sm font-black text-white">SONARA LIVE DECK ENGINE</h2><span className="rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2 py-0.5 text-[8px] font-black text-emerald-300">REAL AUDIO</span></div><p className="mt-1 text-[10px] text-slate-500">Due deck reali con Web Audio: EQ, filter, volume, crossfader, hot cue, loop e tempo sync.</p></div><div className="flex gap-2 text-[8px] font-black"><span className="rounded-lg border border-slate-800 bg-slate-950 px-2 py-1.5 text-slate-400">48k-ready</span><span className="rounded-lg border border-slate-800 bg-slate-950 px-2 py-1.5 text-slate-400">Latency {latency}</span></div></div>
    <div className="mt-5 grid gap-4 xl:grid-cols-2"><DeckPanel id="A" model={deckA} currentTime={timeA} onFile={f => void loadFile('A', f)} onToggle={() => void toggle('A')} onCue={() => cue('A')} onHotCue={i => hotCue('A', i)} onLoop={() => loop('A')} onModel={patch => setDeckA(c => ({ ...c, ...patch }))}/><DeckPanel id="B" model={deckB} currentTime={timeB} onFile={f => void loadFile('B', f)} onToggle={() => void toggle('B')} onCue={() => cue('B')} onHotCue={i => hotCue('B', i)} onLoop={() => loop('B')} onModel={patch => setDeckB(c => ({ ...c, ...patch }))}/></div>
    <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-950/70 p-4"><div className="flex items-center gap-2 text-[10px] font-black text-white"><SlidersHorizontal className="h-4 w-4 text-cyan-300"/> MASTER MIXER</div><div className="mt-4 grid gap-4 lg:grid-cols-[1fr_auto_1fr]"><button onClick={() => sync('A')} className="rounded-xl border border-cyan-500/20 bg-cyan-500/10 px-3 py-2 text-[9px] font-black text-cyan-100">SYNC A → B</button><button onClick={() => { if (audioA.current) audioA.current.currentTime = 0; if (audioB.current) audioB.current.currentTime = 0; setCrossfader(0); }} className="rounded-xl border border-slate-800 bg-slate-900 px-3 py-2 text-slate-400"><RotateCcw className="h-4 w-4"/></button><button onClick={() => sync('B')} className="rounded-xl border border-fuchsia-500/20 bg-fuchsia-500/10 px-3 py-2 text-[9px] font-black text-fuchsia-100">SYNC B → A</button></div><label className="mt-4 block text-[9px] font-black text-slate-500">CROSSFADER <span className="float-right text-white">{crossfader < -0.05 ? 'A' : crossfader > 0.05 ? 'B' : 'CENTER'}</span><input type="range" min={-1} max={1} step={0.01} value={crossfader} onChange={e => setCrossfader(Number(e.target.value))} className="mt-2 w-full accent-fuchsia-500"/></label><label className="mt-3 block text-[9px] font-black text-slate-500">MASTER <span className="float-right text-white">{Math.round(master * 100)}%</span><input type="range" min={0} max={1} step={0.01} value={master} onChange={e => setMaster(Number(e.target.value))} className="mt-2 w-full accent-cyan-500"/></label><div className="mt-3 flex items-center gap-2 text-[9px] text-slate-600"><Gauge className="h-3.5 w-3.5"/> Il tempo sync usa playbackRate reale; non cambia la tonalità indipendentemente come un algoritmo key-lock professionale.</div></div>
  </section>;
}
