...
export default function DJDiagnostics() {
  ...
  const checks = useMemo<Check[]>(() => {
    ...
    return [
      ...
      { label: 'DJ Bridge', ok: bridge.state === 'online', warn: bridge.state === 'connecting', detail: bridge.state === 'online' ? `Bridge ${bridge.bridgeVersion || 'v2'} · RTT ${bridge.latencyMs?.toFixed(1) || '—'} ms · ${bridge.devices.length} device.` : bridge.lastError || `Stato: ${bridge.state}` }
    ];
  ...
  return <section className="rounded-3xl border border-slate-800 bg-slate-900/50 p-4 sm:p-5">
    <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><div className="flex items-center gap-2"><Activity className="h-4 w-4 text-emerald-300"/><h2 className="text-sm font-black text-white">DJ System Diagnostics</h2><span className="rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2 py-0.5 text-[8px] font-black text-emerald-300">{score}% READY</span></div><p className="mt-1 text-[10px] text-slate-500">Controllo browser, audio, hardware API e Bridge locale.</p></div><button onClick={() => { setRefreshKey(value => value + 1); bridge.connect(); }} className="flex items-center justify-center gap-2 rounded-xl border border-slate-700 bg-slate-950 px-4 py-2.5 text-[9px] font-black text-slate-300"><RefreshCw className="h-3.5 w-3.5"/> RICONTROLLA</button></div>
