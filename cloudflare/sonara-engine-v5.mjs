...
async function fetchEngine(env, path, init = {}, timeoutMs = 30000) {
  ...
  try {
    ...
    throw new SonaraEngineError('SONARA network request failed.', 0, true);
  }
}
...
function makeDurations(duration) {
  if (duration <= MAX_SEGMENT_SECONDS) return [duration];
  const count = Math.ceil(duration / MAX_SEGMENT_SECONDS);
  const base = Math.floor(duration / count);
  const remainder = duration - base * count;
  return Array.from({ length: count }, (_, index) => base + (index < remainder ? 1 : 0));
}

function splitLyrics(lyrics, index, count) {
  const text = String(lyrics || '').slice(0, MAX_LYRICS_CHARS).trim();
  ...
  const start = Math.floor((lines.length * index) / count);
  const end = Math.floor((lines.length * (index + 1)) / count);
  return lines.slice(start, end).join('\n').trim();
}

function partDirection(index, count) {
  if (count <= 1) return '';
  if (index === 0) return 'opening section, establish the groove and musical identity, smooth ending for continuation';
  if (index === count - 1) return 'final continuation, preserve the same groove, instrumentation, key and mood, build to a satisfying outro';
  return `continuation ${index + 1} of ${count}, preserve the same groove, instrumentation, key and mood, smooth transitions`;
}

function buildCaption(body, index, count) {
  const caption = [body.genre, body.mood, body.key, body.prompt, partDirection(index, count)]
    .filter(Boolean)
...
function buildGenerationData(spec, body, duration, index, count) {
  ...
  const lyrics = splitLyrics(body.lyrics, index, count);

  setValue(parameters, data, ['Music Caption', 'Caption'], buildCaption(body, index, count));
  setValue(parameters, data, ['Lyrics'], lyrics);
  setValue(parameters, data, ['Instrumental'], !String(body.lyrics || '').trim());
  ...
  // ACE-Step Turbo production profile.
...
async function generate(request, env) {
  ...
  const requestedDuration = clamp(body.durationSec ?? body.duration, 30, 30, 240);

  try {
    const spec = await loadGenerationSpec(env);
    const partDurations = makeDurations(requestedDuration);
    const totalParts = partDurations.length;
    const partData = partDurations.map((duration, index) => buildGenerationData(spec, body, duration, index, totalParts));
    const eventId = await submitGeneration(env, partData[0]);
    const jobId = `v5_${crypto.randomUUID()}`;
    ...
    return json(request, {
      ...
      metadata: {
        ...
        totalParts,
        transport: 'bounded async segments',
        currentStage: totalParts > 1 ? `SONARA: section 1 of ${totalParts} queued` : 'SONARA: generation queued'
      }
    }, 202);
  } catch (error) {
...
function publicAudioUrl(sources) {
  if (!Array.isArray(sources) || sources.length === 0) return '';
  if (sources.length === 1) return `${PUBLIC_API_ORIGIN}/api/modal/audio?source=${encodeURIComponent(sources[0])}`;
  return `${PUBLIC_API_ORIGIN}/api/modal/audio?${sources.map(source => `part=${encodeURIComponent(source)}`).join('&')}`;
}

async function resubmitCurrentPart(env, jobId, context) {
  const currentPart = Number(context.currentPart || 0);
  const retries = Array.isArray(context.retries) ? [...context.retries] : [];
  if (Number(retries[currentPart] || 0) >= 1) return null;

  const data = context.partData?.[currentPart];
      ...
      try {
        while (true) {
          ...
          if (result.state !== 'complete') {
            const retried = await resubmitCurrentPart(env, jobId, context);
            if (retried) {
              context = retried;
              pulse();
              continue;
            ...
            finish({
              ...
              status: 'FAILED',
              progress: 0,
              error: result.error || `SONARA could not complete section ${currentPart + 1} of ${totalParts}.`
            });
            return;
          }
          ...
          if (!source) {
            ...
            finish({ jobId, status: 'FAILED', progress: 0, error: `SONARA completed section ${currentPart + 1} without an audio file.` });
            return;
          }
          ...
          if (nextPart >= totalParts) {
            const audioUrl = publicAudioUrl(sources);
            finish({
              jobId,
              status: 'COMPLETED',
              progress: 100,
          ...
          if (!Array.isArray(nextData)) {
            finish({ jobId, status: 'FAILED', progress: 0, error: 'SONARA continuation data is unavailable.' });
            return;
          }
          ...
          await storeJobContext(jobId, context);
          pulse();
        }
      } catch (error) {
        finish({
...
async function fetchAudioPart(env, source) {
  if (!validSource(source)) throw new SonaraEngineError('Invalid SONARA audio source.', 400, false);
  const cfg = config(env);
  return fetch(`${cfg.baseUrl}${source}`, { method: 'GET', headers: authHeaders(env) });
}
    ...
    async start(controller) {
      try {
        for (const source of parts) {
          const response = await fetchAudioPart(env, source);
...
async function audio(request, env, url) {
  ...
  const source = url.searchParams.get('source') || parts[0] || '';
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path === '/' || path === '/api/health') {
      ...
      return json(request, {
        status: 'HEALTHY',
        service: 'sonara-production-engine-v5',
        engineConfigured: Boolean(cfg.key && cfg.secret),
        engine: 'SONARA',
        transport: 'bounded async segments',
        resilience: 'modal-150s-safe-segments-v6',
